...
<html lang="en">
	<head>
		...
		<title>Driver Benchmarking | Metrobots</title>

		<link
			rel="apple-touch-icon"
			sizes="180x180"
			href="https://metrobots.github.io/assets/favicon/apple-touch-icon.png"
		/>
		<link
			rel="icon"
			type="image/png"
			sizes="32x32"
			href="https://metrobots.github.io/assets/favicon/favicon-32x32.png"
		/>
		<link
			rel="icon"
			type="image/png"
			sizes="16x16"
			href="https://metrobots.github.io/assets/favicon/favicon-16x16.png"
		/>
		<link rel="manifest" href="https://metrobots.github.io/assets/favicon/site.webmanifest" />
		<link
			rel="mask-icon"
			href="https://metrobots.github.io/assets/favicon/safari-pinned-tab.svg"
			color="#5bbad5"
		/>
			...
			function calculateOverallScore(scores) {
				...
				if (scores.reaction && scores.reaction.length > 0) {
					...
					const reactionScore = Math.max(
						0,
						Math.min(33.33, ((600 - bestReaction) / (600 - 150)) * 33.33)
					);
				...
				if (scores.sequence && scores.sequence.length > 0) {
					...
					const sequenceScore = Math.max(
						0,
						Math.min(33.34, ((bestSequence - 1) / (20 - 1)) * 33.34)
					);
