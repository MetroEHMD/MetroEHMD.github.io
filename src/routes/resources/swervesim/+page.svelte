<script>
	...
	class Obstacle {
		...
		checkCollision(robotX, robotY, robotWidth, robotLength, robotHeading) {
			...
			for (const corner of corners) {
				if (
					corner.x >= this.x &&
					corner.x <= this.x + this.width &&
					corner.y >= this.y &&
					corner.y <= this.y + this.height
				) {
					return true;
				}
			}
	...
	class SwerveDrive {
		...
		setDriveInput(xSpeed, ySpeed, rotation, fieldOriented = true) {
			...
			let maxSpeed = this.modules.reduce(
				(max, module) => Math.max(max, Math.abs(module.targetSpeed)),
				0
			);
		...
		update(dt, obstacles) {
			...
			this.modules.forEach((module) => {
				...
				totalVy += vy;
				totalOmega +=
					(-module.y * vx + module.x * vy) / (Math.pow(module.x, 2) + Math.pow(module.y, 2));
			});
			...
			this.x +=
				(totalVx * Math.cos(this.heading) - totalVy * Math.sin(this.heading)) *
				dt *
				PIXELS_PER_METER;
			this.y +=
				(totalVx * Math.sin(this.heading) + totalVy * Math.cos(this.heading)) *
				dt *
				PIXELS_PER_METER;
	...
	onMount(() => {
		...
		function drawRobot() {
			...
			robot.modules.forEach((module, i) => {
				...
				ctx.lineTo(module.forceX * forceScale, module.forceY * forceScale);
				ctx.strokeStyle = '#0f0';
				ctx.lineWidth = 2;
				ctx.stroke();
